...
c_GLSLProgram::c_GLSLProgram(std::string _name, std::string _vertex_shader, std::string _fragment_shader)
{
	Logw("c_GLSLProgram::c_GLSLProgram(): Creating program %s...\n", _name.c_str());
	...
	if (_name != "")
		m_sProgramName = _name;
	else
		m_sProgramName = "GLSL_program_#_" + m_programID;
...
void c_GLSLProgram::AddVariable(int _type, std::string _name, std::string _variable)
{
	...
	*/
	...
	case GLSL_ATTRIBUTE:
		...
		//Logw("ID %d: %s: added GLSL_ATTRIBUTE %s (%s) = %d\n",
		//	m_programID, m_sProgramName.c_str(), _name.c_str(), m_mapAttributes[_name].sVariableName.c_str(), m_mapAttributes[_name].iLocation);
		break;

	case GLSL_UNIFORM:
		m_mapUniforms[_name] = GLSL_variable_location(_variable, glGetUniformLocation(m_programID, _variable.c_str()));
		//Logw("ID %d: %s: added GLSL_UNIFORM %s (%s) = %d\n",
		//	m_programID, m_sProgramName.c_str(), _name.c_str(), _variable.c_str(), m_mapUniforms[_name].iLocation);
		break;
	}
...
void c_GLSLProgram::FinalizeProgram()
{
	...
	Logw("c_GLSLProgram::FinalizeProgram(): %s\n", m_sProgramName.c_str());
	Logw("GLSL_ATTRIBUTE\n");
	...
	{
		Logw("c_GLSLProgram::FinalizeProgram(): %s: Errors encoutered in attribute/uniform locations. Exit.\n\n", m_sProgramName.c_str());
		CloseLogFile();
		exit(EXIT_FAILURE);
	}
	else
		Logw("c_GLSLProgram::FinalizeProgram(): %s: attribute/uniform locations found.\n\n", m_sProgramName.c_str());
